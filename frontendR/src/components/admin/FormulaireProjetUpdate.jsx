...
const FormulaireProjetUpdate = ({
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("priority", updatedProject.priority);
    if (updatedProject.logo instanceof File) {
      formData.append("logo", updatedProject.logo);
    }

    if (Array.isArray(updatedProject.employees)) {
      updatedProject.employees.forEach((employeeId) =>
    ...
    try {
      await axios.post(
        `http://localhost:5001/api/admin/projects/update`,
        formData,
        {
          headers: {
      ...
      onUpdateSuccess && onUpdateSuccess(); // pour recharger dans le parent
      onClose && onClose();
    } catch (err) {
